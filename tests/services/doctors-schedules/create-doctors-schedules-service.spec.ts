...
describe('CreateDoctorsSchedulesService', () => {
  ...
  describe('execute', () => {
    beforeAll(() => {
      ...
      doctorsSchedulesRepository.findExistantSchedules = jest.fn()
    })
    ...
    it('should not be able to create new doctor-schedule if already has a schedule at the same time to same doctor', async () => {
      doctorsSchedulesRepository.findExistantSchedules = jest.fn().mockResolvedValue(doctorScheduleModel)
      ...
      expect(doctorsSchedulesRepository.findExistantSchedules).toHaveBeenNthCalledWith(1, { ...mockDoctorSchedule })
