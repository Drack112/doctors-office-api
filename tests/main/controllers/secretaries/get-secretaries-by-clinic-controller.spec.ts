...
describe('GetSecretariesByClinicController', () => {
  const service = {} as GetSecretariesByClinicService
  ...
  beforeAll(() => {
    ...
    req.originalUrl = '/secretaries-by-clinic/:clinicId'
  })

  describe('handle', () => {
    it('should be able to get a list of secretaries by clinic', async () => {
      service.execute = jest.fn().mockResolvedValue(secretaries)

      await controller.handle(req, res)

      expect(service.execute).toHaveBeenNthCalledWith(1, req.params.clinicId, req.originalUrl)
      expect(res.status).toHaveBeenNthCalledWith(1, 200)
      expect(res.json).toHaveBeenNthCalledWith(1, secretaries)
    })

    it('should not be able to get a list of secretaries by clinic due server error', async () => {
